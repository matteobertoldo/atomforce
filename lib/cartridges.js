        ...
        return del(zippath).then(() => {
            ...
            walker.on('file', (root, filestats, next) => {
                ...
                if (atom.config.get('atomforce.followDots')) {
                    ...
                    if (root.indexOf('.') === -1 && !filestats.name.startsWith('.') && !match([root, filestats.name], filters).length) {
            ...
            archive.outputStream.pipe(fs.createWriteStream(zippath)).on('error', err => {
                props.uploading = false;
