'use babel';

import etch from 'etch';
import { remote } from 'electron';

/** @jsx etch.dom */
export default class TabHeader {
    constructor(props) {
        this.props = props;
        etch.initialize(this);
    }

    render() {
        return (
            <header className="atomforce-tab-header">
                <span className="icon icon-database" />
                <span className="hostname">Files Uploaded</span>
                <button className="button-list-count icon icon-ellipses" onClick={this.showActionsMenu} />
                <button className="button-list-clear icon icon-circle-slash" onClick={this.props.clearAll}>
                    Clear All
                </button>
            </header>
        );
    }

    showActionsMenu(event) {
        event.preventDefault();

        const { Menu, MenuItem } = remote;
        const menu = new Menu();
        const pluralization = this.props.webDAVFilesList.length > 1 ? 's' : '';

        menu.append(
            new MenuItem({
                label: `${this.props.showFileSize ? 'Hide' : 'Show'} file${pluralization} size`,
                click: () => {
                    this.props.manageFilesSize();
                },
                enabled: this.props.webDAVFilesList.length > 0
            })
        );

        menu.append(
            new MenuItem({
                label: 'Show latest 20 files',
                click: () => {},
                enabled: this.props.webDAVFilesList.length > 20
            })
        );

        menu.popup(remote.getCurrentWindow());
    }

    update(props) {
        this.props = props;
        etch.update(this);
    }
}
