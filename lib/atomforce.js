...
export default {
  ...
  createSFCCDebuggerProvider() {
    return {
      getLaunchAttachProvider: (connection) => {
        return new AutoGenLaunchAttachProvider('Atomforce', connection, this.getNodeConfig());
      }
    };
  },
  getNodeConfig() {
    const hostname = {
      name: 'hostname',
      type: 'string',
      description: 'Hostname of the Sandbox',
      required: true,
      visible: true
    };
    ...
    const username = {
      name: 'username',
      type: 'string',
      description: 'Username for login on Sandbox.',
      required: true,
      visible: true
    };
    ...
    const password = {
      name: 'password',
      type: 'string',
      description: 'Password for login on Sandbox.',
      required: true,
      visible: true
    };

    const properties = [hostname, username, password];
    ...
    return {
      launch: {
        ...
        scriptExtension: '.js',
        getProcessName() {
          return 'Script Debugger API 2.0';
        }
      },
      attach: {
        launch: false,
