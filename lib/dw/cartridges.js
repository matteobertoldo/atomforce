...
export default (cartridge, dwdav, props, notify) => {
  ...
  return new Promise((resolve, reject) => {
    ...
    return del(zippath, force).then(() => {
      ...
      walker
        .on('file', (root, filestats, next) => {
          ...
          const data = Array.isArray(utils.watchlist(props)) ? file : rootdata;
    ...
    .then(() => {
      const cleanWebDAV = atom.config.get('atomforce.cartridgesOptions.cleanWebDAV');
      return cleanWebDAV ? dwdav.delete(cartridge, remoteroot).then(() => notify()) : Promise.resolve(notify());
    })
    .then(() => {
