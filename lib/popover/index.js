...
export default class PopoverComponents {
    ...
    async process () {
        ...
        const STEPS = 4; // length of `notify()` callbacks from dw/cartridges.js
        let DEFER = 0;
        ...
        const queue = (task, notify) => {
            return task.reduce((promise, path) => {
                return promise
                    .then(() => {
                        return cartridge(path, dwdav, this.props, notify);
                    })
                    .catch(err => {
                        _data.push(err);
                    });
        ...
        await queue(tasks, notify => {
            ...
            this.props.progress = Math.round((DEFER * 100) / (STEPS * watchlist.length));
            this.update();
        });
